...
- An origin rule performing a `Host` header override will not update the Server Name Indication (SNI) value of the original request; by default, the SNI of the original request is preserved. To set an SNI value different from the `Host` header value, add an [SNI override](#server-name-indication-sni) in the same origin rule or create a separate origin rule for this purpose.
